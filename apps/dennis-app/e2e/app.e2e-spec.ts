...
describe('Starting tests for dennis-app', function() {
  ...
  it('website title should be dennis-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('dennis-app');
    })
  });

  it('network-name should be dennis-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('dennis-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be dennis-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('dennis-app');
    });
  });
